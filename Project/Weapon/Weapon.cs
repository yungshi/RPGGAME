...
public class Weapon : MonoBehaviour
{
    ...
    IEnumerator Shot()//원거리 공격
    {
        ...
        Rigidbody bulletRigid = instantBullet.GetComponent<Rigidbody>();//물리 컴포넌트 부여
        Bullet bulletComp = instantBullet.GetComponent<Bullet>();//총알 컴포넌트
        if (bulletComp != null)
            bulletComp.damage = damage;//무기 데미지를 총알에 전달(적에게 무기 damage가 그대로 적용되도록)
        bulletRigid.linearVelocity = bulletPos.forward * 50;//정면 방향 총알 발사

        yield return null;//대기
        //탄피배출
        GameObject instantCase = Instantiate(bulletCase, bulletCasePos.position, bulletCasePos.rotation);//탄피의 백터 계산
        Rigidbody caseRigid = instantCase.GetComponent<Rigidbody>();
        Vector3 caseVec = bulletCasePos.forward * Random.Range(-3, -2) + Vector3.up * Random.Range(2, 3);
        caseRigid.AddForce(caseVec, ForceMode.Impulse);//충걍량을 주어 탄피를 밀어냄
